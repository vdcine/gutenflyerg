function exportUserData() {
    ...
    const userData = {
        selectedMovie: {
            ...
            ageRating: document.getElementById('edad-sugerida-review')
                .textContent,
        },

        formData: {
            titleReview: document.getElementById('titleInputReview').value,
            synopsisReview: document.getElementById('sinapsisInputReview')
                .value,
            ageRatingReview: document.getElementById('edadSugeridaInputReview')
                .value,
        },
...
function restoreMovieData(movieData) {
    ...
    if (movieData.director) {
        document.getElementById('director-review').textContent =
            movieData.director;
    }
...
function restoreFormData(formData) {
    if (!formData) return;
    document.getElementById('titleInputReview').value =
        formData.titleReview || '';
    document.getElementById('sinapsisInputReview').value =
        formData.synopsisReview || '';
    document.getElementById('edadSugeridaInputReview').value =
        formData.ageRatingReview || '';
}
    ...
    if (designSettings.colors) {
        Object.keys(designSettings.colors).forEach((elementKey) => {
            ...
            if (
                ...
                ['rect2-review', 'tape'].includes(elementKey)
            ) {
        ...
        Object.keys(designSettings.textStrokes).forEach((elementId) => {
            ...
            if (element) {
                element.style.textShadow =
                    designSettings.textStrokes[elementId];
            }
        });
    }
}
...
function restoreImages(imagesData) {
    ...
    if (imagesData.posters && Array.isArray(imagesData.posters)) {
        ...
        if (typeof imagesData.currentPoster === 'number') {
            currentPoster = Math.min(
                imagesData.currentPoster,
                posters.length - 1
            );
        }
